...
func mkFailingClient() *mocks.DistCronClient {
	...
	client.On("RunJobOnThisNode", context.Background(), testJob).Return(nil, dc.InternalError)
	return client
}
...
func TestDispatcherBasics(t *testing.T) {
	...
	rpc.On("GetRpcForNode", ctx, "B").Return(mkClient("B"), nil).Once()
	...
	rpc.On("GetRpcForNode", ctx, "B").Return(mkFailingClient(), nil).Once()
	handle, err = disp.NewJob(ctx, testJob)
	require.EqualError(t, err, dc.NoNodesAvailableError.Error())
	...
	<-telemetryUpdate
	rpc.On("GetRpcForNode", ctx, "A").Return(mkClient("A"), nil).Once()
